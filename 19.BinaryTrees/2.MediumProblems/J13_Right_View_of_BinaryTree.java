...
public class J13_Right_View_of_BinaryTree {
    ...
    public static List<Integer> rightView(Node root){
        ...
        for(int i=0; i<ds.size(); i++){
            ls.add(ds.get(i).get(ds.get(i).size()-1));
        }
    ...
    //* Optimal : 
    public static void recursionRight(Node root, int level, List<Integer> ls){
        if(root==null){
            return;
        }
        ...
        recursionRight(root.right, level+1, ls);

        recursionRight(root.left, level+1, ls);
    }
    public static List<Integer> Right_View(Node root){
        List<Integer> ls = new ArrayList<>();
        if(root==null) return ls;

        recursionRight(root, 0, ls);
    ...
    public static void main(String[] args) {
        ...
        //* Optimal : 
        List<Integer> ans = Right_View(root);
        System.out.println(ans);
        //? Time Complexity : O(N)
        //? Space Complexity : O(height) [stack space] {for balanced tree => O(logN)}
    }
